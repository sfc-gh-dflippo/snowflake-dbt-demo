...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Deploy dbt project to Snowflake", add_help=False
    )
    ...
    if not project_name:
        logging.error(
            "Project name required via -n/--name or SNOWFLAKE_DBT_PROJECT env var"
        )
        sys.exit(1)
    ...
    if not shutil.which("snow"):
        logging.error(
            "Snowflake CLI not found. Install with: pip install snowflake-cli"
        )
        sys.exit(1)
    ...
    try:
        ...
        run_command(
            ["dbt", "parse", "--profiles-dir", "deploy_config"], "dbt parse failed"
        )
