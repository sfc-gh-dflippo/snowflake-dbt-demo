...
async function cloneOrPullRepo(url: string, targetPath: string): Promise<{ changed: boolean, commit: string }> {
  if (fs.existsSync(path.join(targetPath, '.git'))) {
    ...
    if (changed) {
      console.log(`  Updated: ${oldCommit?.slice(0, 7)} → ${newCommit?.slice(0, 7)}`);
    } else {
...
function scanAllSkills(projectRoot: string, repoPaths: Record<string, string>): {
...
function formatSkillsSection(localSkills: Record<string, Skill>, repoSkills: Record<string, Skill>): string {
  const lines: string[] = [];
  lines.push('## MCP-Managed Skills\n\n');
  lines.push('This project uses the **Skills MCP Server** to dynamically manage both local SKILL.md files and skills from remote Git repositories.\n\n');
  lines.push('# Skills\n\n');
  lines.push('**What are Skills?**\n\n');
  lines.push('Skills are structured instruction sets that enhance AI assistant capabilities for specific domains or tasks. Each skill is a folder containing:\n');
  ...
  for (const [name, skill] of Object.entries(repoSkills)) {
    if (!(name in localSkills)) {  // Skip overridden
...
async function main(): Promise<void> {
  ...
  const { localSkills, repoSkills: allRepoSkills } = scanAllSkills(SCRIPT_DIR, repoPaths);
  ...
  const overridden = Object.keys(localSkills).filter(name => name in allRepoSkills);
  ...
  const uniqueRepoSkills = Object.keys(allRepoSkills).filter(name => !(name in localSkills));
  const total = Object.keys(localSkills).length + uniqueRepoSkills.length;
  console.log(`\n✓ Synced ${total} total skills to AGENTS.md`);
  console.log(`  (${Object.keys(localSkills).length} local, ${uniqueRepoSkills.length} from repos)`);
}
