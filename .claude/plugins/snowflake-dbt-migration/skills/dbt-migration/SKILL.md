...
### SnowConvert AI (Required for Supported Platforms)

**IMPORTANT**: For all supported platforms listed below, SnowConvert AI is a **hard requirement** —
do not attempt manual conversion. SnowConvert AI handles edge cases, platform-specific idioms, and
dependency resolution that manual translation consistently misses.
...
**CRITICAL**: Migrated dbt models must preserve the original SQL object names from the source
database. Do not rename tables, views, or columns during migration.

### Rules

1. **Keep original object names**: The final dbt model name (and therefore the resulting Snowflake
   table/view name) must match the original source object name. Use dbt `alias` config if the file
   name must differ from the target object name.
2. **Add intermediate CTEs — do not rename**: When breaking complex logic into steps, add CTEs
   within the model rather than creating new model names. If intermediate models are necessary, use
   `int_` prefix but keep the base name intact (e.g., `int_original_name__step1`).
3. **Preserve column names**: Output columns must retain their original names. If a column needs
   transformation, apply it in a CTE and alias back to the original name in the final SELECT.
4. **Document name deviations**: If a name change is unavoidable (e.g., reserved word conflicts),
   add a comment documenting the original name and the reason for the change.

### Rationale

Name retention ensures that downstream consumers (reports, applications, APIs) continue to work
without modification after migration. It also simplifies validation by allowing direct name-based
...
**SnowConvert AI Requirement**: For supported platforms, SnowConvert AI provides extraction scripts
that automate object inventory, dependency mapping, and initial code conversion. Use it — do not
skip this step for supported platforms.
