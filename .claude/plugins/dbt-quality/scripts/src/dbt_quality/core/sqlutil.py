...
def line_col(content: str, position: int) -> tuple[int, int]:
    """1-based ``(line, column)`` for a character offset."""
    line_start = content.rfind("\n", 0, position) + 1
    return content[:position].count("\n") + 1, position - line_start + 1


def span(content: str, start: int, end: int) -> dict[str, int]:
    """
    Position keywords for ``make_suggestion``, as
    ``**span(model.raw, match.start(), match.end())``.

    Returns a dict rather than a tuple so a call site cannot transpose the four
    values, and so a rule that gains a span is a one-line change. ``end_column``
    is exclusive, matching VS Code's ``endColumn``, and a zero-width match is
    widened to one character so it still produces a visible range.

    Offsets may come from ``raw`` or ``stripped`` interchangeably -- the
    stripping functions blank characters in place, so the two strings share a
    coordinate system. See :func:`raw_span`.

    **The end is clamped to the start line.** A match crossing a newline reports
    to end-of-line instead of spanning the construct, because a range covering a
    60-line CTE body highlights the whole screen and reads worse than a range on
    its first line. The ceiling is that multi-line constructs under-report their
    extent; to lift it, drop the clamp and let ``end_line`` exceed ``line`` --
    the schema and the lint format already carry both.
    """
    line, column = line_col(content, start)
    line_end = content.find("\n", start)
    if line_end == -1:
        line_end = len(content)
    stop = min(max(end, start + 1), line_end)
    return {
        "line": line,
        "column": column,
        "end_line": line,
...
def extract_ctes(stripped_sql: str) -> list[dict[str, Any]]:
    ...
    ``start``/``end`` bound the CTE *name*, not its body, so a rule reporting a
    badly-named CTE highlights the name rather than the whole block.
...
def extract_macro_defs(raw: str) -> list[dict[str, Any]]:
    """
    Macro definitions in a file, with name, args, body, line, and offsets.
    ...
    ``start``/``end`` bound the macro *name*. ``body_start`` is the body's
    absolute offset in ``raw``, which a rule needs to convert a body-relative
    match offset back into a file position -- a column computed against the body
    alone is wrong whenever the body starts part-way through a line.
    """
    out: list[dict[str, Any]] = []
