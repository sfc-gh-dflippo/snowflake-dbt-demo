...
INC_TRUNCATE_LOAD = "INC001"
INC_DELETE_LOAD = "INC002"
INC_FOLDER_HOOK_DML = "INC003"
INC_MANUAL_INCREMENTAL = "INC004"
INC_FULL_RELOAD = "INC005"
INC_NO_GUARD = "INC006"
INC_NO_UNIQUE_KEY = "INC007"
INC_NO_STRATEGY = "INC008"
INC_NO_WATERMARK = "INC009"
INC_HOOK_WRITES_ELSEWHERE = "INC010"
INC_MERGE_EXCLUDE_MISMATCH = "INC011"
INC_APPEND_ON_MUTABLE = "INC012"
INC_UNBOUNDED_SCAN = "INC013"
INC_EXTERNAL_LOADER = "INC015"
...
@rule(
    ...
    rationale=(
        "dbt rebuilds a table declaratively. Truncating the relation in a hook and "
        "reloading it reimplements ETL-era batch loading, defeats --full-refresh "
        "semantics, and leaves the table empty if the model then fails."
    ),
)
def truncate_and_load(model: ModelFile, project: ProjectContext) -> Iterator[Finding]:
    for key, body in config_hooks(model.effective_config):
        for match in TRUNCATE_PATTERN.finditer(body):
            ...
            yield make_finding(
                INC_TRUNCATE_LOAD,
                f"{key} truncates this model's own relation before the model rebuilds it.",
                file=model.relative_path,
                evidence=f'{key}="{body.strip()[:180]}"',
                remediation=(
                    "Delete the hook. If the goal is a periodic clean rebuild, use "
                    "`materialized='table'` and let dbt replace the relation atomically; "
        ...
        for match in DROP_PATTERN.finditer(body):
            ...
            yield make_finding(
                INC_TRUNCATE_LOAD,
                f"{key} drops this model's own relation; dbt already manages its lifecycle.",
                file=model.relative_path,
                evidence=f'{key}="{body.strip()[:180]}"',
                remediation=(
                    "Remove the hook. Use `dbt build --full-refresh --select "
                    f"{model.name}` when the relation genuinely needs rebuilding."
                ),
                effort=Effort.LOW,
            )
...
@rule(
    ...
    severity=Severity.ERROR,
    rationale=(
        "dbt's delete+insert and merge strategies express row replacement declaratively "
        "and atomically. A DELETE in a hook runs outside that contract, is invisible to "
        "the DAG, and can leave the table short of rows if the insert then fails."
    ),
)
def delete_and_load(model: ModelFile, project: ProjectContext) -> Iterator[Finding]:
    for key, body in config_hooks(model.effective_config):
        for match in DELETE_PATTERN.finditer(body):
            if not _targets_self(match.group(1), model):
                continue
            reads_sibling = bool(re.search(r"\bref\s*\(", body))
            message = f"{key} deletes rows from this model's own relation."
            if reads_sibling:
                message += (
                    " The hook also reads another model via ref(), a dependency dbt "
                    "cannot see when ordering the DAG."
                )
            yield make_finding(
                INC_DELETE_LOAD,
                message,
...
@rule(
    ...
    rationale=(
        "A model should produce exactly one relation. A hook inserting into a different "
        "table creates a write dbt does not track, cannot test, and will not rebuild in "
        "the right order."
    ),
)
def hook_writes_elsewhere(
    ...
    for key, body in config_hooks(model.effective_config):
        for match in INSERT_PATTERN.finditer(body):
            ...
            yield make_finding(
                INC_HOOK_WRITES_ELSEWHERE,
                f"{key} performs `{match.group(1).lower()}` into `{target.strip('();')}`, "
                "a relation dbt does not know this model produces.",
                file=model.relative_path,
                evidence=f'{key}="{body.strip()[:180]}"',
                remediation=(
                    "Make the target its own dbt model so the write is declarative and "
                    "appears in the DAG. If it is genuinely operational bookkeeping "
                    "(an audit or control table), move it to an on-run-end hook so it "
...
@rule(
    ...
    rationale=(
        ...
        "folder. A TRUNCATE there wipes data written by the models that ran before it, "
        "so results depend on execution order."
    ),
)
def folder_hook_dml(project: ProjectContext) -> Iterator[Finding]:
    ...
    for scope_path, key, body in walk(project.project_yml.get("models") or {}, []):
        ...
        yield make_finding(
            INC_FOLDER_HOOK_DML,
            f"`{key}` on `{scope_path}` performs {verb}. It executes once per model in "
            "that folder, so the second model undoes the first model's work.",
...
def manual_incremental(model: ModelFile, project: ProjectContext) -> Iterator[Finding]:
    ...
    line = find_line_number(model.stripped, match.start()) if match else None
    yield make_finding(
        INC_MANUAL_INCREMENTAL,
        "Model materializes as a full table but filters rows to a relative date "
        "window, so each run reprocesses the window and discards anything outside it.",
        file=model.relative_path,
        line=line,
...
@rule(
    INC_FULL_RELOAD,
    "Model is named or configured as a full reload",
    category=CATEGORY,
    scope=Scope.MODEL,
    severity=Severity.WARNING,
    rationale=(
        "A name or config declaring a full reload usually marks a table that outgrew "
        "full refresh but was never converted, and it forecloses incremental processing."
    ),
)
def full_reload(model: ModelFile, project: ProjectContext) -> Iterator[Finding]:
    name_match = FULL_RELOAD_NAME_PATTERN.search(model.name)
    if name_match:
        yield make_finding(
            INC_FULL_RELOAD,
            f"Model name `{model.name}` declares a full reload strategy.",
            file=model.relative_path,
            evidence=model.name,
            remediation=(
                ...
                "genuinely required, drop the suffix and say why in the model "
                "description so the choice reads as deliberate."
            ),
            effort=Effort.MEDIUM,
        )
    ...
    if model.effective_config.get("full_refresh") is True:
        yield make_finding(
            INC_FULL_RELOAD,
            "`full_refresh: true` forces a complete rebuild on every run, which "
            "disables incremental processing entirely.",
            file=model.relative_path,
...
@rule(
    ...
    severity=Severity.ERROR,
    rationale=(
        "Without an is_incremental() branch the model reads its full source on every "
        "run, so the incremental materialization adds merge cost without saving any "
        "scan -- and with merge it can rewrite every row each run."
    ),
)
def no_incremental_guard(
    ...
    yield make_finding(
        INC_NO_GUARD,
        "Model is materialized as incremental but never calls `is_incremental()`, "
        "so it processes the entire source on every run.",
        file=model.relative_path,
...
@rule(
    ...
    severity=Severity.ERROR,
    rationale=(
        "merge and delete+insert need a unique_key to know which rows to replace. "
        "Without one the model appends, so any reprocessed row silently duplicates."
    ),
)
def no_unique_key(model: ModelFile, project: ProjectContext) -> Iterator[Finding]:
    ...
    yield make_finding(
        INC_NO_UNIQUE_KEY,
        f"Incremental model uses the `{strategy}` strategy but defines no "
        "`unique_key`, so rows cannot be matched and reprocessing duplicates them.",
        file=model.relative_path,
...
def no_strategy(model: ModelFile, project: ProjectContext) -> Iterator[Finding]:
    if not model.is_incremental:
        return
    ...
    if strategy is None:
        yield make_finding(
            INC_NO_STRATEGY,
            "Incremental model relies on the adapter default rather than declaring "
    ...
    if str(strategy).lower() not in VALID_STRATEGIES:
        yield make_finding(
            INC_NO_STRATEGY,
            f"`incremental_strategy='{strategy}'` is not a recognised Snowflake strategy.",
            severity=Severity.ERROR,
            file=model.relative_path,
...
@rule(
    ...
    severity=Severity.WARNING,
    rationale=(
        "An is_incremental() branch that does not consult {{ this }} has no watermark: "
        "it cannot know what has already been loaded, so it either reprocesses a fixed "
        "window or misses late rows."
    ),
)
def no_watermark(model: ModelFile, project: ProjectContext) -> Iterator[Finding]:
    if not model.is_incremental or not IS_INCREMENTAL_PATTERN.search(model.raw):
        return
    if _reads_stream(model):
        return  # streams are self-tracking; a watermark would be redundant
    for match in re.finditer(
        ...
        yield make_finding(
            INC_NO_WATERMARK,
            "The `is_incremental()` branch never references `{{ this }}`, so the load "
            "has no watermark against what is already in the table.",
            file=model.relative_path,
...
def unbounded_scan(model: ModelFile, project: ProjectContext) -> Iterator[Finding]:
    if not model.is_incremental:
        return
    if _reads_stream(model):
        return
    for match in re.finditer(
        ...
        yield make_finding(
            INC_UNBOUNDED_SCAN,
            "The incremental predicate uses only a `max()` watermark with no absolute "
            "floor, so the source scan is unbounded.",
            file=model.relative_path,
            line=find_line_number(model.raw, match.start()),
...
@rule(
    ...
    "merge_exclude_columns without a matching passthrough branch",
    category=CATEGORY,
    scope=Scope.MODEL,
    severity=Severity.WARNING,
    rationale=(
        "merge_exclude_columns preserves an insert-time value on update, but only if "
        "the model still selects that column. Excluding a column the model never "
...
def merge_exclude_mismatch(
    ...
    for column in columns:
        ...
        yield make_finding(
            INC_MERGE_EXCLUDE_MISMATCH,
            f"`merge_exclude_columns` lists `{name}`, but the model never selects that "
            "column, so it will be NULL on insert and never populated.",
            file=model.relative_path,
...
@rule(
    ...
    severity=Severity.WARNING,
    rationale=(
        "append never updates an existing row. If the source can restate a record, "
        "append accumulates duplicate versions with no way to tell which is current."
    ),
)
def append_on_mutable(model: ModelFile, project: ProjectContext) -> Iterator[Finding]:
    if not model.is_incremental:
        return
    ...
    yield make_finding(
        INC_APPEND_ON_MUTABLE,
        f"Model appends, but selects `{match.group(1)}` -- a column implying rows are "
        "restated over time. Appends will accumulate duplicate versions.",
        file=model.relative_path,
        line=find_line_number(model.stripped, match.start()),
        evidence=match.group(0).strip(),
        remediation=(
            "If rows can change, switch to `incremental_strategy='merge'` with the "
            "business `unique_key`. If the model is genuinely an immutable event "
            "stream and the column is only carried through, say so in the model "
            "description so the append is reviewable."
        ),
        effort=Effort.MEDIUM,
    )
...
@rule(
    INC_EXTERNAL_LOADER,
    "Table is loaded by a task or procedure outside dbt",
    category=CATEGORY,
    scope=Scope.PROJECT,
    severity=Severity.WARNING,
    rationale=(
        "A procedure or task writing to a dbt-managed relation competes with dbt for "
        "ownership. dbt will overwrite it, tests will describe a state nothing "
        "guarantees, and lineage is broken."
    ),
)
def external_loader(project: ProjectContext) -> Iterator[Finding]:
    ...
    for path, raw in project.loose_sql:
        ...
        for match in INSERT_PATTERN.finditer(stripped):
            ...
            yield make_finding(
                INC_EXTERNAL_LOADER,
                f"`{path.name}` defines a task or procedure that writes to `{target}`, "
                "which is also a dbt model. Two owners are writing the same relation.",
                file=relative,
                line=find_line_number(stripped, match.start()),
