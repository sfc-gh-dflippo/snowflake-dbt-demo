...
> **Do NOT begin manual conversion of tables or views.** Sybase IQ has **partial SnowConvert AI
> support** (tables and views only — stored procedures and functions are NOT supported). Before
> writing any dbt model for a table or view by hand:
>
...
> source object name (lowercased, without schema prefix). For example, `dbo.patient_visits` becomes
> `patient_visits.sql`. Do not rename objects during migration unless explicitly requested by the
> user.
