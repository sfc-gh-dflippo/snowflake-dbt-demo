...
last_scraped: '2026-01-14T16:56:39.444118+00:00'
scraper_version: 1.1.0
source_url: https://docs.snowflake.com/en/sql-reference/functions/array_contains.html
title: ARRAY_CONTAINS | Snowflake Documentation
---
...
[Reference](../../reference.md)[Function and stored procedure reference](../../sql-reference-functions.md)[Semi-structured and structured data](../functions-semistructured.md)ARRAY\_CONTAINS

Categories:
:   [Semi-structured and structured data functions](../functions-semistructured) (Array/Object)

# ARRAY\_CONTAINS[¶](#array-contains "Link to this heading")
...
:   Value to find in `array`.
    ...
    * If `array` is a [semi-structured array](../data-types-semistructured.html#label-data-type-array), `value_expr` must evaluate to a
      [VARIANT](../data-types-semistructured.html#label-data-type-variant).
    * If `array` is a [structured array](../data-types-structured), `value_expr` must evaluate
      to a type that is [comparable](../data-types-structured.html#label-structured-types-compare-struct-struct) to the type of the array.
...
For more information about NULL values in arrays, see [NULL values](../../user-guide/semistructured-considerations.html#label-variant-null).

## Usage notes[¶](#usage-notes "Link to this heading")

* The function does not support wildcards in `value_expr`. However, you can
  use the [ARRAY\_TO\_STRING](array_to_string) function to convert an array to a string, then search the
  string with wildcard characters. For example, you can specify wildcards to search the
  returned string using the [[ NOT ] LIKE](like) and [REGEXP\_LIKE](regexp_like) functions.
* If `array` is a semi-structured array, [explicit casting](../data-type-conversion.html#label-data-type-explicit-casting)
  of the `value_expr` value to a VARIANT value is required for values of the following data types:

  + [String & binary](../data-types-text)
  + [Date & time](../data-types-datetime)
