...
### To_Char(datetime)[¶](#to-char-datetime)
...
## MAX KEEP DENSE_RANK[¶](#max-keep-dense-rank)
...
## MIN KEEP DENSE_RANK[¶](#min-keep-dense-rank)
...
The SQL format model used to parse the input _`expr`_ and return. For more information, see [SQL Format Models](https://docs.snowflake.com/en/sql-reference/sql-format-models).
...
The number of fractional decimal digits (from 0 to _`precision`_ - 1). 0 indicates no fractional digits (i.e. an integer number). The default scale is 0.
