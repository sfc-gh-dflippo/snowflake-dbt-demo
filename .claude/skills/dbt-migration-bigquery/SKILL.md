...
> **Do NOT begin manual conversion of tables or views.** BigQuery has **partial SnowConvert AI
> support** (tables and views only — procedures and functions are NOT supported). Before writing any
> dbt model for a table or view by hand:
>
...
>    (check the conversion report for EWIs) or for procedures/functions (which require manual
>    conversion).
>
...
> source object name (lowercased, without project/dataset prefix). For example,
> `my_project.my_dataset.patient_visits` becomes `patient_visits.sql`. Do not rename objects during
> migration unless explicitly requested by the user.
